...
# This dictionary will store weather data for locations we've already fetched.
weather_cache = {}

# --- ANALYSIS FUNCTION (No changes here) ---
def analyze_data(weather_data, target_date_str):
    """
    Analyzes historical weather data to calculate probabilities, averages,
...
# --- MODIFIED `get_historical_weather` function ---
def get_historical_weather(location_name):
    # --- (NEW) CACHING LOGIC ---
    # 1. Check if we have already fetched data for this location.
    ...
    try:
        # 2. If not in cache, fetch from the geocoding API.
        ...
        # 3. Fetch from the historical weather API.
        ...
        # 4. (NEW) Store the successful result in the cache before returning.
...
# --- MODIFIED `analyze_weather` endpoint ---
...
def analyze_weather():
    ...
    # Note: `get_historical_weather` now only needs the location.
    weather_data, lat, lon, error_message = get_historical_weather(location)
    
    if error_message:
        # Return a 503 Service Unavailable error for rate limiting issues.
